...
class UtilityAI {
   ...
   */
  score(barb, state, role, strategy) {
  ...
  // ---------------------------------------------------------------------------
  scoreAttack(d, player, playerCommitted, facingPlayer, aggressionMult) {
    let score = 0;
    let reasoning = '';
    if (d < ATTACK_RANGE) {
  ...
  scoreKick(d, player, playerJustAttacked, facingPlayer, aggressionMult) {
    let score = 0;
    let reasoning = '';
  ...
  scoreFlank(d, barb, player, role, state) {
    if (role !== 'FLANKER') {
      return {
        action: 'FLANK',
        score: 0,
  ...
  scoreJump(d, player, facingPlayer) {
  ...
  scoreBlock(d, player, playerCommitted, barb, role) {
  ...
  scoreRetreat(d, barb, player, role, strategy) {
    ...
    const retreatDir = -(Math.sign(player.position.x - barb.position.x) || 1);
    return {
      action: 'RETREAT',
      score: 0.68,
...
// ---------------------------------------------------------------------------
function strategyAggressionMultiplier(strategy) {
  switch (strategy) {
    case 'AGGRESSIVE':
    case 'OVERWHELM':
      return 1.2;

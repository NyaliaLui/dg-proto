...
class BaitPunishSM {
   ...
   */
  tick(barb, player, role, claudeSuggestsBait) {
    const now = Date.now();
    ...
    switch (this.state) {
      ...
      case 'INACTIVE': {
        ...
        if (role === 'BAITER' && claudeSuggestsBait && d < 3.0) {
          ...
          logger_1.logger.debug(`[BaitSM ${barb.id}] INACTIVE → POSITIONING`);
        }
      ...
      case 'POSITIONING': {
        ...
        this.baitStartedAt = now;
        logger_1.logger.debug(`[BaitSM ${barb.id}] POSITIONING → BAIT_ACTIVE`);
      ...
      case 'PUNISH_READY': {
        ...
        if (elapsed >= PUNISH_DELAY_MS) {
          ...
          logger_1.logger.debug(`[BaitSM ${barb.id}] PUNISH_READY → PUNISHING`);
  ...
  // ---------------------------------------------------------------------------
  reset(id, reason) {
    logger_1.logger.debug(`[BaitSM ${id}] reset — ${reason}`);
    this.state = 'INACTIVE';
    this.playerAttackedAt = null;
  }
  decision(barbarianId, action, direction, durationMs, strategyTag, reasoning) {
    return { action, direction, durationMs, strategyTag, reasoning };
  }
}
exports.BaitPunishSM = BaitPunishSM;
//# sourceMappingURL=BaitPunishSM.js.map

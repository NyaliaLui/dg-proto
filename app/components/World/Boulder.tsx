...
function Boulder({ x, id, onRegister, onUnregister }: BoulderProps) {
  ...
  const boulderHalfHeight = halfHeight + radius; // 0.9 — same total height as Paladin
  const boulderHalfWidth = radius * 2; // 1.0 — twice the Paladin's diameter
  ...
  // THREE.js raycaster intersects visible=false meshes — this does not affect rendering.
  ...
  return (
    <RigidBody type="fixed" position={[x, boulderHalfHeight * 0.4, 0]}>
      <mesh castShadow receiveShadow scale={[1, boulderHalfHeight / boulderHalfWidth, 1]}>
        <icosahedronGeometry args={[boulderHalfWidth, 1]} />
        <meshStandardMaterial color="#778899" flatShading />
      </mesh>
      {/* Invisible box — exact collider dimensions — used only for sword raycast */}
      <mesh ref={raycastBoxRef} visible={false}>
        <boxGeometry args={[boulderHalfWidth * 2, boulderHalfHeight * 2, boulderHalfWidth * 2]} />
        <meshBasicMaterial />
      </mesh>
      <CuboidCollider

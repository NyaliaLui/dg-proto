...
export function Barbarian({ id, onDeath, settings }: BarbarianProps) {
  ...
  const jumpAnim = getAnimation(useFBX(CHARACTER_DEFAULTS.ANIMATIONS.JUMP));
  const leftBlockAnim = getAnimation(
    useFBX(CHARACTER_DEFAULTS.ANIMATIONS.LEFT_BLOCK),
  );
  ...
  useFrame((_state, delta) => {
    ...
    if (rigidBodyRef.current) {
      ...
      if (jumpPendingRef.current && isGroundedRef.current) {
        ...
        yVelocityRef.current = CHARACTER_DEFAULTS.JUMP.VELOCITY;
      ...
      if (!isGroundedRef.current) {
        ...
        yVelocityRef.current -= CHARACTER_DEFAULTS.JUMP.GRAVITY * clampedDelta;
        const t = rigidBodyRef.current.translation();
